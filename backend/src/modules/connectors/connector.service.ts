...
export async function syncConnector(connectorId: string, range?: SyncRange, onProgress?: (progress: SyncProgress) => void) {
  ...
  try {
    ...
    await prisma.connector
      .update({ where: { id: connectorId }, data: { syncStartedAt: new Date(), syncPhase: 'starting', syncMessage: 'Contacting the platform' } })
      .catch(() => undefined);

    const reportProgress = (progress: SyncProgress) => {
      onProgress?.(progress);
      // Fire-and-forget: the sync must not wait on, or fail because of, a
      // progress write.
      void persistProgress(connectorId, progress);
    };
      ...
      async (tx) => {
        ...
        await tx.connector.update({
          ...
          data: {
            ...
            // Terminal state: clear the live progress so the UI stops
            // reporting a sync that has finished.
            syncStartedAt: null,
            syncPhase: 'completed',
            syncMessage: `${events.length.toLocaleString()} rows saved`,
    ...
    await prisma.connector.update({
      ...
      data: {
        ...
        syncStartedAt: null,
        syncPhase: 'failed',
        syncMessage: message.slice(0, 200),
      },
    });
...
/**
 * Mirrors progress onto the connector row.
 *
 * The in-memory map is still the fast path for the client that started the
 * sync; this is what lets a different page, a different tab, or a client that
 * reconnects after a restart see the same thing. Failures are swallowed: a
 * progress write must never be able to fail a sync.
 */
  ...
  await prisma.connector
    .update({
      ...
      data: {
        ...
        syncMessage: progress.message ?? null,
        syncCompleted: progress.completed ?? null,
        syncTotal: progress.total ?? null,
        ...(progress.phase === 'completed' || progress.phase === 'failed' ? { syncStartedAt: null } : {}),
      },
    })
    .catch(() => undefined);
}
...
export async function listConnectors(workspaceId: string) {
  const connectors = await prisma.connector.findMany({
    ...
    select: {
      ...
      lastRowCount: true, coverageStart: true, coverageEnd: true,
      syncStartedAt: true, syncPhase: true, syncMessage: true,
      syncCompleted: true, syncTotal: true,
    },
    orderBy: { createdAt: 'asc' },
  });

  // A sync older than this was almost certainly killed by a restart.
  // Reporting it as still running would leave a spinner on the page forever.
  const STALE_AFTER_MS = 20 * 60_000;
  const now = Date.now();

  return connectors.map((connector) => {
    const running =
      Boolean(connector.syncStartedAt) && now - connector.syncStartedAt!.getTime() < STALE_AFTER_MS;

    return {
      ...connector,
      // Derived so every client agrees on what "syncing" means, rather than
      // each one reimplementing the staleness rule.
      syncing: running,
      syncProgress: running
        ? {
            phase: connector.syncPhase ?? 'running',
            message: connector.syncMessage ?? 'Working',
            completed: connector.syncCompleted ?? null,
            total: connector.syncTotal ?? null,
            startedAt: connector.syncStartedAt!.toISOString(),
            elapsedMs: now - connector.syncStartedAt!.getTime(),
          }
        : null,
    };
  });
