...
export const ga4Connector: ConnectorAdapter = {
  ...
  async sync(credentials, days = 30, range?: SyncRange, selected?: SelectedFields, onProgress?: (progress: SyncProgress) => void) {
    ...
    const { REPORTS_PER_BATCH, BATCHES_IN_FLIGHT, MAX_REPORTS_PER_SYNC, QUOTA_FLOOR } = GA4_LIMITS;
    ...
    const plannedTasks = buildTasks(dimensions, metrics);
    const queue: ReportTask[] = plannedTasks.slice(0, MAX_REPORTS_PER_SYNC);
    const skippedTasks = plannedTasks.length - queue.length;
    ...
    onProgress?.({
      ...
      message: skippedTasks
        ? `Preparing ${initialTotal.toLocaleString()} GA4 report groups (${skippedTasks.toLocaleString()} deferred to a later sync)`
        : `Preparing ${initialTotal.toLocaleString()} GA4 report groups`,
      warning: skippedTasks ? 'The report budget deferred some fields to protect the GA4 property quota.' : undefined,
    });
    ...
    const runSingle = async (task: ReportTask) => {
      try {
        ...
        // GA4 can reject one metric in an otherwise valid chunk because a
        // custom metric is incompatible with the selected dimension. Retry
        // the chunk one metric at a time so valid fields are not discarded
        // with the incompatible one.
        if (task.metrics.length > 1) {
          for (const metric of task.metrics) {
            await runSingle({ ...task, metrics: [metric], offset: 0 });
          }
          return;
        }
    ...
    if (quotaExhausted) {
      throw new Error(`GA4 sync stopped before saving because ${quotaMessage || 'the API quota was exhausted'}. Existing stored data was left unchanged. Requested ${startDate}..${endDate}.`);
    }
