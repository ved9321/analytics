...
// *wanted*. The same four rows make a fine bar chart or a fine composition
// chart — which one answers the question is not visible in the rows.

describe('the question drives the chart type', () => {
  const categorical = [
    { campaign: 'A', cost: 400, conversions: 40 },
    { campaign: 'B', cost: 300, conversions: 20 },
    { campaign: 'C', cost: 200, conversions: 12 },
    { campaign: 'D', cost: 100, conversions: 5 },
  ];
  const catPlan = plan({ intent: 'breakdown', groupBy: 'campaign', metrics: ['cost'] });

  it('answers a share question with a composition chart', () => {
    const decision = decideChart(catPlan, { grouped_by: 'campaign', rows: categorical }, 'what share of spend does each campaign take?');
    expect(decision.type).toBe('stackedBar');
    expect(decision.normalised).toBe(true);
  });

  it('answers a relationship question with a scatter', () => {
    const twoMetrics = plan({ intent: 'breakdown', groupBy: 'campaign', metrics: ['cost', 'conversions'] });
    expect(decideChart(twoMetrics, { grouped_by: 'campaign', rows: categorical }, 'how do cost and conversions relate to each other?').type).toBe('scatter');
  });

  it('answers a ranking question with bars, not a composition', () => {
    expect(decideChart(catPlan, { grouped_by: 'campaign', rows: categorical }, 'show me the top 3 campaigns').type).toBe('bar');
  });

  it('lets the data overrule an impossible request', () => {
    // Two rows is not a composition, one metric is not a correlation, and
    // categorical rows are not a time series.
    const two = [{ campaign: 'A', cost: 400 }, { campaign: 'B', cost: 300 }];
    expect(decideChart(catPlan, { grouped_by: 'campaign', rows: two }, 'what share does each take?').type).not.toBe('stackedBar');
    expect(decideChart(catPlan, { grouped_by: 'campaign', rows: categorical }, 'how do these correlate?').type).not.toBe('scatter');
    expect(decideChart(catPlan, { grouped_by: 'campaign', rows: categorical }, 'what is the trend over time?').type).not.toBe('line');
  });

  it('still decides from shape alone when no question is given', () => {
    expect(decideChart(catPlan, { grouped_by: 'campaign', rows: categorical }).type).not.toBe('none');
  });
});
