...
const checks = [
  {
    ...
    // Matched loosely on purpose. The first version required the exact
    // argument list and reported a false failure the moment a further
    // argument was added — a check that cries wolf gets ignored, which is
    // worse than no check.
    must: ['enabledFields(connector.id)'],
    pattern: /adapter\.sync\([^)]*\bselected\b/,
  },
  {
...
for (const check of checks) {
  ...
  if (check.pattern && !check.pattern.test(source)) missing.push(String(check.pattern));
  if (missing.length) {
